...
export const createCart = async () => {
    ...
    const cart = await prisma.cart.create({
        data: {
            ...
            userId: user?.id || null, // Fixed: Use userId instead of user connect
        },
        include: {
            items: true,
        }
...
export const updateCartItem = async (
    ...
    revalidatePath("/");
    return getOrCreateCart(cart.id); // Fixed: Use cart.id instead of cartId
}
...
export const syncCartWithUser = async (cartId: string | null) => {
    ...
    if(!existingUserCart) {
        const newCart = await prisma.cart.update({
            ...
            data: {
                userId: user.id // Fixed: Use userId instead of user connect
            },
            include: {
                items: true,
            }
    ...
    for(const item of existingAnonymousCart.items) {
        const existingItem = existingUserCart.items.find((userItem) => userItem.sanityProductId === item.sanityProductId); // Fixed: Changed variable name to avoid conflict
    ...
    revalidatePath("/");
    return existingUserCart; // Fixed: Return existingUserCart directly instead of calling getOrCreateCart
}
